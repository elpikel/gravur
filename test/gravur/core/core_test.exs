defmodule Gravur.Core.CoreTest do
  ...
  describe "greeting" do
    test "get_all_greetings/1 get all greetings related to book", %{
      user: user,
      template: template
    } do
    ...
    test "get_all_greetings/1 does not get unrelated grretings to book", %{
      user: user,
      template: template
    } do
      {:ok, book} = Core.create_book(create_book(@valid_attrs, user, template))
  ...
  describe "book" do
    ...
    test "get_all_books/1 get all books related to user with templates", %{
      user: user,
      template: template
    } do
    ...
    test "get_book_with_greetings/1 returns book without greetings", %{
      user: user,
      template: template
    } do
      {:ok, book} = Core.create_book(create_book(@valid_attrs, user, template))
    ...
    test "get_book_with_greetings/1 returns book with greetings", %{
      user: user,
      template: template
    } do
      {:ok, book} = Core.create_book(create_book(@valid_attrs, user, template))
